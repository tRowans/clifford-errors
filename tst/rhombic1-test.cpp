...
{   
    ...
    EXPECT_EQ(lattice.edgeToFaces[865][0], 1);
    EXPECT_EQ(lattice.edgeToFaces[987][0], 1);
}
...
TEST(buildQubitIndicesTestR1, CorrectOutput)
{
    ...
    rhombic::r1::buildQubitIndices(lattice.outerQubitIndices, 
                                    lattice.innerQubitIndices, lattice.allQubitIndices, 6);
    ...
    vint innerQubitIndicesExpected = {6,18,19,37,42,108,110,111,127,128, 
                                      ...
                                      238,239,253,254,256,257,258,261,275,
                                      327,346,351,363,364};
...
TEST(buildZSyndIndicesTestR1, CorrectOutput)
{
    ...
    vint zSyndIndicesExpected = {148,150,151,168,169,171,178,196,197,198,
                                 ...
                                 913,914,919,1011,1012,1013,1014,1032,
                                 1033,1034,1039,1059,1060,1061,1062,1152,
                                 1180,1200};
...
TEST(buildDefectIndicesTestR1, CorrectOutput)
{
    vint defectIndices;
    rhombic::r1::buildDefectIndices(defectIndices, 6);

    vint defectIndicesExpected = {37,49,79,91,252,253,258,259,264,265};
    EXPECT_EQ(defectIndices, defectIndicesExpected);
}

//------------------------------------------------------------

TEST(buildXLogicalTestR1, CorrectOutput)
{
    Lattice lattice(6);
    rhombic::r1::buildFaces(lattice);
    rhombic::r1::buildQubitIndices(lattice.outerQubitIndices,
                                   ...
                                   lattice.L);
    rhombic::r1::buildXLogical(lattice);

    vint xLogicalExpected = {0,216,219,6,108,110,111,
                             131,218,222,225,239,327};
    EXPECT_EQ(lattice.xLogical, xLogicalExpected);
}

//------------------------------------------------------------

TEST(buildZLogicalTestR1, CorrectOutput)
{
    Lattice lattice(6);
    rhombic::r1::buildFaces(lattice);
    rhombic::r1::buildZLogicals(lattice);
    vvint zLogicalsExpected = {{0,129,36},
                              {111,18,147},
                              {6,135,42},
                              {219,126,255},
                              {108,237,144},
