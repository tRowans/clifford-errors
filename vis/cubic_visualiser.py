...
#------------------------------#
#----------!SET THESE!----------#
path = "/Users/tom/Documents/clifford-errors/vis/test_csv/"
L = 2
#------------------------------#
...
def face_to_vertex_coords(face, L):
    direction = face % 3
    vertices = np.array([index_to_coord(face//3, L) for i in range(4)])
    if (direction == 0):
...
def edge_to_vertex_coords(edge, L):
    direction = edge % 3
...
def build_vertices(bm, vertices):
    for vertex in vertices:
        coord = index_to_coord(vertex, L)
        bm.verts.new((coord[0], coord[1], coord[2]))

def build_edges(bm, edges):
    for edge in edges:
        edge = [mathutils.Vector(index_to_coord(v, L)) for v in edge]
...
def build_defects(bm, defects):
    for defect in defects:
        translation = mathutils.Matrix.Translation(index_to_coord(defect, L))
        bmesh.ops.create_uvsphere(bm, u_segments=32, v_segments=16, diameter=0.1, matrix=translation)
...
try:
    ...
    bpy.context.collection.objects.delete(cube)
except:
    pass
