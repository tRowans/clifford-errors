...
void buildCellToFaces(vvint &cellToFaces, vvint &vertexToFaces, vvint &faceToVertices, int L)
{
    ...
    //I do these separately and not as part of buildFaces 
    //because I want them to be in a specific order
    //this is useful later when making paths through faces 
    //on dual lattice for Z error decoding
...
void buildDefectIndices(vint &defectIndices, int L)
{
    for (int v = 0; v < L * L * L; v++)
    {
        ...
        if ((cdw0.xi[0] + cdw0.xi[1] + cdw0.xi[2]) % 2 == 0)
        {
...
void buildLattice(Lattice &lattice)
{
    ...
    buildXLogical(lattice);  //this needs to come after index building functions
