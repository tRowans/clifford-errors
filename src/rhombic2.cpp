...
coord cellToCoord(int cell, int L)
{
    ...
    if ((cd.xi[0] + cd.xi[1] + cd.xi[2]) % 2 == 1) cd = indexToCoord(2*cell + 1, L);
    return cd;
}
...
int neighXYZ(int v, int dir, int sign, int L)
{
    ...
    if (sign > 0)
    {
        ...
        else if (dir == 1) c.xi[1] = (c.xi[1] + 1) % (5*L);
        else if (dir == 2) c.xi[2] = (c.xi[2] + 1) % L;
    }
    else if (sign < 0)
    {
        if (dir == 0) c.xi[0] = (c.xi[0] - 1 + L) % L;
        else if (dir == 1) c.xi[1] = (c.xi[1] - 1 + 5*L) % (5*L);
...
void buildSlice(vint &lowerQubitIndices, vint &middleQubitIndices, vint &upperQubitIndices, vint &syndIndicesZ, vint &syndIndicesXLayer, vint &defectIndices, vint &bulkSweepVertices, vint &middleBoundarySweepVertices, vint &zLogical, vint &xLogicalFragment1, vint &xLogicalFragment2, vvint &vertexToFaces, vvint &faceToVertices, vvint &faceToEdges, vpint &edgeToVertices, int L)
{
    ...
    while(!generatorVertices.empty())
    {
        ...
        //boundary Z stabilisers
        //don't want to add ones with support only on lower layer
        if (c.xi[2] == (L-3) && c.xi[0] < (L-2))
        {
            if (c.xi[0] < (L-3))
            {
                syndIndicesZ.push_back(4*neigh(v,xz,1,L) + xy);
                syndIndicesZ.push_back(4*(v+1-L) + yz);
                syndIndicesZ.push_back(4*(v+1) + xyz);
                if (c.xi[0] > 1) defectIndices.push_back(neigh(v,xz,1,L));
            }
            if (c.xi[0] > 1)
            {
                syndIndicesZ.push_back(4*v + xz);
            }
        }
        if (c.xi[2] == 1 && c.xi[0] < (L-2))
        {
            if (c.xi[0] < (L-3))
            {
                syndIndicesZ.push_back(4*neigh(v,xy,1,L) + xz);
                syndIndicesZ.push_back(4*neigh(v,xy,1,L) + xyz);
            }
            if (c.xi[0] > 1)
            {
                syndIndicesZ.push_back(4*v + xy);
                syndIndicesZ.push_back(4*neigh(v,xy,1,L) + yz);
                defectIndices.push_back(v);
            }
        }
        
        //X stabilisers
        if (c.xi[2] != (L-3) && c.xi[0] != L-2) syndIndicesXLayer.push_back((neigh(v,xz,1,L) + L)/2);
    ...
    for (std::pair<int, int> p : edgeCount)
    {
        if (p.second == 3)
        {
            syndIndicesZ.push_back(p.first);
        }
    }
    ...
    for (std::pair<int, int> p : lowerEdgeCount)
    {
        if (p.second == 3)
        {
            syndIndicesZ.erase(std::find(syndIndicesZ.begin(), syndIndicesZ.end(), p.first));
        }
    }
